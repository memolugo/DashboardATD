    ...
    return L.divIcon({
      ...
      html: `
        <div class="${isSelected ? 'selected-marker-pulse' : ''}" style="
          ...
          position: relative;
        ">
          <svg width="${isSelected ? '16' : '12'}" height="${isSelected ? '16' : '12'}" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="4">
  ...
  return (
    <div className="w-full h-full relative group">
      ...
      <button 
        ...
        className="absolute top-6 right-6 z-[1000] bg-white shadow-lg p-2.5 rounded-xl hover:bg-gray-50 transition-colors border border-gray-100"
        title="Centrar en Morelos"
      >
      ...
      {isLoading && (
        <div className="absolute inset-0 z-[2000] bg-white/60 backdrop-blur-md flex items-center justify-center">
          <div className="flex flex-col items-center gap-4">
            <div className="w-10 h-10 border-4 border-[#8c3154] border-t-transparent rounded-full animate-spin" />
            <span className="text-[10px] font-bold text-[#8c3154] uppercase tracking-widest">Sincronizando...</span>
          </div>
        </div>
      )}
    </div>
