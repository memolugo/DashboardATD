...
export const parseCSV = (csvText: string): SheetMarker[] => {
  ...
  return lines.slice(1).map((line, idx) => {
    ...
    return {
      ...
      horario: rawData['horario'] || 'Lunes a Viernes 08:00 - 15:00',
      numero: rawData['numero'] || '777 329 2200',
      category: secRaw.toLowerCase().includes('salud') ? 'Salud' : 
                secRaw.toLowerCase().includes('educacion') ? 'Educación' : 'Oficina General',
      image: rawData['image'] || 'https://images.unsplash.com/photo-1577412647305-991150c7d163?q=80&w=2070&auto=format&fit=crop'
    };
